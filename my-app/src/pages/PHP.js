import React from 'react';
import "./styles/PHP.css"
function PHP() {
  return (
    <div id="phpDiv">
    <li className="ProjectNames">MVVC PHP Project</li>
    <a href="webpages/php/coxaden7/index.php" className='webLinks'>MVVC PHP Project</a>
    <p className="ProjectText">This php project involved turning our project structure into the mvvc layout. The project itself connects to and manages a database.</p>
   
    <li className="ProjectNames">User Management</li>
    <a href="webpages/php/coxaden8/index.php" className='webLinks'>User Management</a>
    <p className="ProjectText">This php project added on to the previous one to create a database to manage new users logging into the site.</p>
    </div>
  );
}

export default PHP;
